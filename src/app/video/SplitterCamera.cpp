...
void SplitterCamera::_webCamDetectFunc(MSWebCamManager *manager) {
  auto sourceCamera = ms_web_cam_manager_get_cam(
      manager, SplitterCamera::_cameraId.toStdString().c_str());
  if (sourceCamera == nullptr) {
    qWarning()
        << "The SplitterCamera for" << SplitterCamera::_cameraId
        << "was not created, because the source camera couldn't be found";
    return;
  }

  auto camera =
      ms_web_cam_manager_create_cam(manager, &SplitterCamera::description);
  ...
  splitterCameraState->splitterSource = ms_factory_create_filter_from_desc(
      manager->factory, &SplitterSource::description);

  SplitterSource::State *sourceState =
      (SplitterSource::State *)splitterCameraState->splitterSource->data;
  ...
  ms_filter_link(sourceState->sizeconv, 0, splitterCameraState->splitterSource,
                 0);
  ...
  ms_filter_call_method(sourceState->cameraReader, MS_FILTER_SET_VIDEO_SIZE,
                        &size);
...
void SplitterCamera::_webCamUninitFunc(MSWebCam *camera) {
  SplitterCamera::State *splitterCameraState =
      reinterpret_cast<SplitterCamera::State *>(camera->data);
...
// static
bool_t SplitterCamera::_webCamEncodeToMimeType(MSWebCam *camera,
                                               const char *mimeType) {
...
MSFilter *SplitterCamera::State::createSplitterSink() {
  ...
  MSFilter *sink =
      ms_factory_create_filter_from_desc(factory, &SplitterSink::description);
