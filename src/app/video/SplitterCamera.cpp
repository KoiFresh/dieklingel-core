...
void SplitterCamera::_webCamDetectFunc(MSWebCamManager *manager) {
    auto sourceCamera = ms_web_cam_manager_get_cam(
        manager,
        SplitterCamera::_cameraId.toStdString().c_str()
    );
    if (sourceCamera == nullptr) {
        qWarning(
        ) << "The SplitterCamera for"
          << SplitterCamera::_cameraId
...
MSFilter *SplitterCamera::_webCamCreateReaderFunc(MSWebCam *camera) {
    SplitterCamera::State *splitterCamera =
        (SplitterCamera::State *)camera->data;
...
// static
bool_t SplitterCamera::_webCamEncodeToMimeType(
