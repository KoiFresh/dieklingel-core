...
Gpio::Gpio(std::shared_ptr<QJSEngine> engine) : _engine(engine) {
...
void Gpio::input(int pin, QJSValue callback) {
    if (this->_outputs.contains(pin)) {
        throw std::invalid_argument(
            QString("A pin should only be used once. The pin %1 is already "
                    "declared as output.")
                .arg(pin)
                .toStdString()
        );
...
QJSValue Gpio::output(int pin) {
    ...
    if (!this->_outputs.contains(pin)) {
        if (this->_isSetupCompleted) {
            throw std::invalid_argument(
                QString("An output should be configured during startup before "
