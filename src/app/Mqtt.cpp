...
Mqtt::Mqtt(QString address) {
  ...
  this->_mqtt = std::make_shared<mqtt::async_client>(
      address.toStdString(), QUuid().toString().toStdString(), nullptr);
...
void Mqtt::connect(QString username, QString password) {
  ...
  auto message = mqtt::message("dieklingel/core/system/state", "disconnected",
                               mqtt::GRANTED_QOS_2, true);
  ...
  QtConcurrent::run([this, options]() {
    ...
    for (;;) {
      try {
        ...
        token->wait();
        this->_mqtt->publish("dieklingel/core/system/state", "connected",
                             mqtt::GRANTED_QOS_2, true);
        ...
        qWarning()
            << QString(
                   "Failed to connect to the mqtt broker %1. Reason: %2. "
                   "Establish a connection will be tried again in %3 seconds.")
                   .arg(this->_address)
...
void Mqtt::message_arrived(mqtt::const_message_ptr message) {
  this->messageReceived(QString(message->get_topic().c_str()),
