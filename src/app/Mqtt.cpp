...
Mqtt::Mqtt(QString address)
{
	...
	this->_mqtt = std::shared_ptr<mqtt::async_client>(new mqtt::async_client(address.toStdString(), "core", nullptr));
	...
	QtConcurrent::run(
		[this, options]()
		{
			...
			for (;;)
			{
				...
				{
					const auto time = 10;
					qWarning() << QString("Failed to connect to the mqtt broker %1. Reason: %2. Establish a connection will be tried again in %3 seconds.")
									  .arg(this->_address)
