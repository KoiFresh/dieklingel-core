...
void App::_initCore() {
    ...
    auto address =
        factory->createAddress(this->_settings.getCoreSipAddress().toStdString()
        );
    auto info = factory->createAuthInfo(
        address->getUsername(),
        "",
        this->_settings.getCoreSipPassword().toStdString(),
        "",
        "",
        ""
    );
    ...
    this->_core->setCaptureDevice(
        this->_settings.getCoreSipCaptureDevice().toStdString()
    );
    this->_core->setPlaybackDevice(
        this->_settings.getCoreSipPlaybackDevice().toStdString()
    );
...
void App::snapshot() {
    ...
    if (call != nullptr)
    {
            qInfo() << "We received a request to capture a snapshot! Taking
    snapshots, when call is active is currently not supported."; return;
    }*/
...
void App::printCoreInformation() {
    ...
    info << "qml enabled: "
         << (this->_settings.getCoreQmlEnabled() ? "true" : "false")
         << Qt::endl;
    if (this->_settings.getCoreQmlEnabled()) {
    ...
    info << "mqtt enabled: "
         << (this->_settings.getCoreMqttEnabled() ? "true" : "false")
         << Qt::endl;
    if (this->_settings.getCoreMqttEnabled()) {
    ...
    info << "sip enabled:"
         << (this->_settings.getCoreSipEnabled() ? "true" : "false")
         << Qt::endl;
    if (this->_settings.getCoreSipEnabled()) {
        ...
        info << "\t\t- config: "
             << Factory::get()->getConfigDir(nullptr).c_str() << Qt::endl;
        info << "\t\t- data: " << Factory::get()->getDataDir(nullptr).c_str()
             << Qt::endl;
        info << "\t\t- data resources: "
             << Factory::get()->getDataResourcesDir().c_str() << Qt::endl;
        info << "\t\t- download: "
             << Factory::get()->getDownloadDir(nullptr).c_str() << Qt::endl;
        info << "\t\t- image resources: "
             << Factory::get()->getImageResourcesDir().c_str() << Qt::endl;
        info << "\t\t- mediastreamer plugins: "
             << Factory::get()->getMspluginsDir().c_str() << Qt::endl;
        info << "\t\t- ring resources: "
             << Factory::get()->getRingResourcesDir().c_str() << Qt::endl;
        info << "\t\t- sound resources: "
             << Factory::get()->getSoundResourcesDir().c_str() << Qt::endl;
        info << "\t\t- top resources: "
             << Factory::get()->getTopResourcesDir().c_str() << Qt::endl;
        info << "\tports:" << Qt::endl;
...
void App::onRegistrationStateChanged(
    const std::shared_ptr<Core> &lc,
